...
- **Fuse-capped export power** — the discharge limit commanded during an export slot is capped at `(fuse rating − safety buffer) × 3 × 230 V + house load`. The plant's 14.4 kW discharge limit is never commanded during export: a 20 A main fuse has a 13.8 kW ceiling.
