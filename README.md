...
- **Export spike threshold (per kWh)** — price spread above the period's cheapest hour at or above which a slot may become an export slot (the same spread convention as the discharge threshold, so it adapts automatically when the overall price level shifts). Set to 0 to disable (default). Configured in the setup wizard's economics step or later via **Configure** → battery options.
