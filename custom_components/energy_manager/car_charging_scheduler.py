...
def build_car_charging_schedule(
    ...
    Args:
        ...
        fallback_mode: If True, select cheapest half of available slots
            instead of exact N (for unrecognized vehicles).
    ...
    # Step 3: Select charge slots
    if fallback_mode:
        # EV-08: select cheapest half of available slots
        sorted_by_price = sorted(available, key=lambda s: s.price)
        midpoint = len(available) // 2
        charge_ids = {id(s) for s in sorted_by_price[:midpoint]}
    else:
        # Normal mode: select cheapest ceil(hours_needed) slots
        slots_needed = math.ceil(hours_needed) if hours_needed > 0 else 0
        sorted_by_price = sorted(available, key=lambda s: s.price)
        charge_ids = {id(s) for s in sorted_by_price[:slots_needed]}
