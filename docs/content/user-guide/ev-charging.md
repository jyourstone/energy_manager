...
- **Planned Charge Power** *(per car, diagnostic)* — the kW the scheduler used to size that car's slots. Attributes carry `learned_power_kw` (the measured throughput, empty until there is one), `ceiling_power_kw` (the **Max Charge Power** number), `phase_capability` (the car's configured phase count, which is the bucket the planner reads), and `source` — `learned` once EM has a measurement for that car and phase count, `ceiling` while it has none. When `source` is `learned` but the state equals `ceiling_power_kw`, the measurement came out above the ceiling and was capped. See [Measured Charge Power](#measured-charge-power).
