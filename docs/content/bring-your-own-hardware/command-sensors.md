...
| Battery commanded charge limit | Maximum battery charge power in kW (fuse-limited; during solar charging tracks live PV surplus — PV production minus house consumption — not gross PV, so opportunistic charging never pulls extra power from the grid) | Write it to your inverter's charge-power limit |
...
!!! note "House consumption entity must exclude battery draw"
    The house-consumption sensor you point Energy Manager at (used to compute PV surplus for opportunistic solar charging) must report consumption that excludes the battery's own charging power — otherwise a charging battery inflates the reading and understates the real surplus available.
